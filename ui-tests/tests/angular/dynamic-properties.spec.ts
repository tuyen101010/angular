...
describe('DYNAMICPROPERTIES', () => {
    ...
    describe('CRUD', () => {
        ...
        it('should save record when "Save" button is clicked', async () => {
            await Utils.fillInputs({ 'input[name=Name]': 'test' });
            await Utils.selectOptionByValue('.modal[aria-labelledby=createOrEditModal].show select', 'SINGLE_LINE_STRING');
        ...
        it('should save changes to record when "Save" button is clicked', async () => {
            await Utils.fillInputs({ 'input[name=Name]': 'changed_name' });
            await Utils.saveForm();
            await Utils.waitForResponse();
