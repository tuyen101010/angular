...
namespace I3T.CRM.Localization
{
    ...
    public class LanguageAppService : CRMAppServiceBase, ILanguageAppService
    {
        ...
        public async Task<PagedResultDto<LanguageTextListDto>> GetLanguageTexts(GetLanguageTextsInput input)
        {
            ...
            // Add custom
            List<TenantCustomPageItemDto> customItems = TenantCustomPagesCache.GetList(AbpSession.TenantId);
            if (customItems != null && customItems.Count > 0)
            {
                List<string> iAllKey = languageTexts.Select(s => s.Key).ToList();
                foreach (TenantCustomPageItemDto cItem in customItems)
                {
                    List<LanguageTextListDto> iAllText = languageTexts.ToList();

                    if (!String.IsNullOrEmpty(cItem.PageMenuKey))
                    {
                        if (!iAllKey.Contains(cItem.PageMenuKey))
                        {
                            iAllKey.Add(cItem.PageMenuKey);
                            iAllText.Add(new LanguageTextListDto { Key = cItem.PageMenuKey });
                        }
                    }

                    if (!String.IsNullOrEmpty(cItem.PageTitleKey))
                    {
                        if (!iAllKey.Contains(cItem.PageTitleKey))
                        {
                            iAllKey.Add(cItem.PageTitleKey);
                            iAllText.Add(new LanguageTextListDto { Key = cItem.PageTitleKey });
                        }
                    }

                    languageTexts = iAllText.AsQueryable();
                }
            }
